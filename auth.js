...
router.post('/register', async (req, res) => {
    try {
        ...
        sendEmail(
            ...
            'Welcome to Eurasia News media!',
            '<h1>Welcome!</h1><p>Thank you for registering on Eurasia News. We are glad to see you, a new reader.</p>'
        );
...
router.post('/login', passport.authenticate('local', {
    successRedirect: '/home', // Redirect to the secure home page upon successful login
    failureRedirect: '/login', // Redirect back to the login page if there is an error
    failureFlash: true // Allow flash messages for error handling
}));
...
router.post('/delete-publication/:id', async (req, res) => {
    try {
        const { id } = req.params;
        await Publication.findByIdAndDelete(id); // Delete the publication by its ID
...
router.get('/edit-publication/:id', async (req, res) => {
    try {
        ...
        res.render('edit-publication', { publication }); // Render the edit form with the publication data
    } catch (error) {
...
router.post('/update-publication/:id', async (req, res) => {
    ...
    try {
        ...
        res.redirect('/home'); // Redirect after updating the publication
    } catch (error) {
...
router.get('/contact', (req, res) => {
    res.render('contact', { title: 'Contact Us', apiKey: 'YOUR_GOOGLE_MAPS_API_KEY' }); // Provide your Google Maps API key
})
